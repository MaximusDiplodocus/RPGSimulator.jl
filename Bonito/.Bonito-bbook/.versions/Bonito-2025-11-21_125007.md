...
```
```julia (editor=true, logging=false, output=true)
using Bonito, DataFrames, Plots
using RPGSimulator
using Base64

# Forcer le backend GR
Plots.gr()
...
# Préparer les données PV par round
round_numbers = sort(unique([log.round for log in logs]))
jed_pv = Int[]
hussin_pv = Int[]

for r in round_numbers
    logs_round = [log for log in logs if log.round == r]
    push!(jed_pv, logs_round[end].actor_PV)
    push!(hussin_pv, logs_round[end].target_PV)
end

df_pv = DataFrame(Round = round_numbers, Jed_PV = jed_pv, Hussin_PV = hussin_pv)

# -----------------------------
# Créer le graphique avec Plots.gr
# -----------------------------
plt = Plots.plot(
    df_pv.Round,
    [df_pv.Jed_PV df_pv.Hussin_PV],
    labels=["Jed" "Hussin"],
    xlabel="Round",
    ylabel="Points de vie",
    lw=3,
    color=[:green :red],
    title="Évolution des PV par round"
)

# Exporter le SVG en Base64
svg_string = Plots.svg(plt)
svg_base64 = Base64.base64encode(svg_string)
data_url = "data:image/svg+xml;base64,$svg_base64"

# Affichage dans BonitoBook
Bonito.DOM.div(
    Bonito.DOM.h2("Graphique des PV par round"),
    Bonito.DOM.img(src=data_url)
)
