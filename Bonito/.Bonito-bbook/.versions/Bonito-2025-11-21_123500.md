...
using Bonito, DataFrames, StatsPlots
using RPGSimulator
...
# -----------------------------
rounds = [log.round for log in logs if log.action in ["attack", "skill"]]
pv_archer = [log.actor_PV for log in logs if log.actor == archer.nom]
pv_gobelin = [log.actor_PV for log in logs if log.actor == gobelin.nom]

df_pv = DataFrame(
    Round = rounds,
    Jed_PV = pv_archer,
    Hussin_PV = pv_gobelin
)

# -----------------------------
# Affichage du tableau dans Bonito
# -----------------------------
Bonito.DOM.div(
...
Bonito.DOM.div(
    ...
    Bonito.Plot(df_pv.Round, [df_pv.Jed_PV df_pv.Hussin_PV], 
        labels=["Jed" "Hussin"], xlabel="Round", ylabel="PV", 
        line_width=2)
)
