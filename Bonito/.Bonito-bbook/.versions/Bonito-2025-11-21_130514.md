...
for r in rounds
    logs_round = [log for log in logs if log.round == r]
    # Trouve les PV à la fin du round
    jed_end = maximum([log.actor_PV for log in logs_round if log.actor == "Jed"])
    gob_end = maximum([log.actor_PV for log in logs_round if log.actor == "Hussin"])
    push!(jed_pv, jed_end)
    push!(gob_pv, gob_end)
end
